...
class cranet(nn.Module):
    ...
    def __init__(
        ...
        self.inc_pool = nn.AdaptiveAvgPool2d(1)
    ...
    def forward(
        ...
        inc_descriptor = torch.cat([
            self.inc_pool(inc_28).flatten(1),
            self.inc_pool(inc_14).flatten(1),
            self.inc_pool(inc_7).flatten(1),
        ], dim=1)
        ...
        if return_embedding and return_inc_embedding:
            return logits, embedding, inc_embedding
        if return_embedding:
